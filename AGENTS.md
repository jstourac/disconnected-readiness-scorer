...
- `no_image_tags.py` — Enforces `@sha256:` digest refs; rejects mutable tags. Source code and manifest files produce blocker severity. Skips directories managed by params.env + kustomize. Filters to git-tracked files only. HTTP/HTTPS URLs are excluded from image detection.
- `no_runtime_egress.py` — Detects outbound HTTP calls in Go/Python/TS/shell source. Distinguishes hardcoded URLs (blocker) from configurable ones (info). Filters to git-tracked files only.
